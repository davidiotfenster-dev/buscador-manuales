...
function inicializarLaboratorioIntegral() {
  ...
  const state = {
    ...
    posicionObjetivo: null, // Si viene de la app
    estadoMotor: "parado", // "parado", "subiendo", "bajando"
    averiaActiva: null, // "fase_invertida", "corte_termico", "wifi_incompatible", "cpulsar_bus_cortado", "bloqueo_mecanico"
    wifiActivo: true,
  ...
  const LAB_CATALOGO_DISPOSITIVOS = {
    "connect-1": {
      ...
      descripcion: "<strong class='text-iot-tealLight'>Connect-1:</strong> Alimentación directa 230V con relés integrados para 1 motor de 4 hilos. Antena Wi-Fi integrada en placa.",
      hasBus: false
    },
    "connect-2": {
      titulo: "CONNECT-2",
      subtitulo: "Doble Canal / Sensores · 230V",
      descripcion: "<strong class='text-iot-tealLight'>Connect-2:</strong> Dispone de salidas independientes para 2 motores o entradas auxiliares de sensores meteorológicos (viento / lluvia).",
      hasBus: false
    },
    "c-wall": {
      titulo: "C-WALL",
      subtitulo: "Mecanismo Empotrado 60mm · Táctil 230V",
      descripcion: "<strong class='text-amber-400'>C-Wall (Neutro Obligatorio):</strong> Receptor inteligente empotrable. Debe disponer de Fase y Neutro directos en la caja de mecanismo de 60 mm.",
      hasBus: false
    },
    "c-pulsar": {
      titulo: "C-PULSAR",
    ...
    "connect-evo": {
      titulo: "CONNECT EVO",
      subtitulo: "Mecanismo Superficie de Marco · 230V",
      descripcion: "<strong class='text-iot-tealLight'>Connect EVO:</strong> Mecanismo de marco con teclas mecánicas basculantes integradas y enclavamiento mecánico de seguridad.",
      hasBus: false
    }
  };

  // Catálogo de Textos y Asesoría Técnica de Averías
  const LAB_DIAGNOSTICOS_AVERIAS = {
    "fase_invertida": {
      icono: "🔀",
      titulo: "Avería Canónica: Fases de Maniobra Invertidas (Marrón ↔ Negro)",
      mensaje: "Los hilos marrón (subida) y negro (bajada) están conectados a la inversa en la salida del motor. Al pulsar SUBIR, el motor desciende la persiana. Solución recomendada: Pulsa el botón 'Swap Giro' o permuta los bornes marrón y negro en la regleta de salida.",
      tipo: "alerta"
    },
    "corte_termico": {
      icono: "⚡",
      titulo: "Avería Canónica: Corte de Suministro / Disparo de Magnetotérmico",
      mensaje: "El circuito de 230V AC se encuentra sin alimentación eléctrica. Los instrumentos marcan 0.0V y 0.0W. La persiana se detiene inmediatamente y la app móvil pasa a estado 'Sin Conexión'. Solución: Rearma la palanca del magnetotérmico C10.",
      tipo: "peligro"
    },
    "wifi_incompatible": {
      icono: "📶",
      titulo: "Avería Canónica: Wi-Fi en Banda 5 GHz o CG-NAT de Operador",
      mensaje: "El router está emitiendo en banda exclusiva de 5 GHz o el proveedor de fibra aplica CG-NAT con aislamiento de clientes. La app pierde conexión con la nube ('Offline'). Sin embargo, ¡el pulsador físico de pared de 230V sigue funcionando de forma 100% autónoma en obra!",
      tipo: "advertencia"
    },
    "cpulsar_bus_cortado": {
      icono: "🪛",
    ...
    "bloqueo_mecanico": {
      icono: "🛑",
      titulo: "Avería Canónica: Bloqueo Mecánico / Sobrecarga del Motor",
      mensaje: "La persiana se ha acuñado en las guías laterales. La potencia absorbida se dispara a más de 330W. Se ha activado la protección térmica de seguridad del motor para evitar la destrucción del bobinado. Solución: Desatasca las lamas y deja enfriar el motor.",
      tipo: "peligro"
    },
    "nominal": {
      icono: "💡",
      titulo: "Estado Nominal del Sistema",
      mensaje: "Tensión nominal 230V AC presente, relés abiertos en reposo (0.8 W standby). Puedes pulsar las teclas del interruptor de pared, accionar la app en el smartphone virtual o activar averías canónicas para probar el diagnóstico SAT.",
      tipo: "normal"
    }
  };

  // ---------- Control de Movimiento de la Persiana ----------
  function iniciarManiobra(direccion) {
    ...
    const subiendoReal = state.sentidoInvertido
      ? (direccion === "bajar")
      : (direccion === "subir");

    if (subiendoReal && state.posicionPersiana >= 100) {
      if (scadaFcStatus) scadaFcStatus.textContent = "Final de carrera SUPERIOR activo (100% Abierta)";
      pararManiobra();
      return;
    }
    if (!subiendoReal && state.posicionPersiana <= 0) {
      if (scadaFcStatus) scadaFcStatus.textContent = "Final de carrera INFERIOR activo (0% Cerrada)";
      pararManiobra();
      return;
    }

    state.estadoMotor = direccion;
    if (direccion === "subir") {
  ...
  // ---------- Bucle de Animación y Física Continua ----------
  function bucleAnimacion(timestamp) {
    ...
    if (state.estadoMotor !== "parado" && state.alimentacion230v && state.averiaActiva !== "bloqueo_mecanico") {
      const deltaSeg = deltaMs / 1000.0;
      const incremento = state.velocidadPctPorSegundo * deltaSeg;

      const subiendoReal = state.sentidoInvertido
        ? (state.estadoMotor === "bajar")
        : (state.estadoMotor === "subiendo");

      if (subiendoReal) {
        state.posicionPersiana += incremento;
        if (state.posicionPersiana >= 100) {
          state.posicionPersiana = 100;
          pararManiobra();
          if (scadaFcStatus) scadaFcStatus.textContent = "Final de carrera SUPERIOR alcanzado (100%)";
        }
      } else {
        state.posicionPersiana -= incremento;
        if (state.posicionPersiana <= 0) {
          state.posicionPersiana = 0;
          pararManiobra();
          if (scadaFcStatus) scadaFcStatus.textContent = "Final de carrera INFERIOR alcanzado (0%)";
        }
      }

      if (state.posicionObjetivo !== null) {
        if (Math.abs(state.posicionPersiana - state.posicionObjetivo) < 1.5) {
  ...
  // ---------- Actualización Visual de la Persiana SVG ----------
  function actualizarVistaPersiana() {
    const pct = Math.max(0, Math.min(100, state.posicionPersiana));
    const alturaLuz = 450;
    const alturaLamas = alturaLuz - (pct / 100.0) * (alturaLuz - 15);

    if (svgSlatsBg) {
      svgSlatsBg.setAttribute("height", Math.max(10, alturaLamas));
    }
    if (svgSlatsZocalo) {
  ...
  // ---------- Actualización General de Toda la Interfaz ----------
  function actualizarVista() {
    ...
    if (state.alimentacion230v) {
      ...
      if (wireInN) wireInN.style.opacity = "1";

      const voltFluctuacion = (230.2 + (Math.random() * 0.6 - 0.3)).toFixed(1);
      if (telemetriaVoltaje) telemetriaVoltaje.textContent = voltFluctuacion + " V";

      if (state.averiaActiva === "bloqueo_mecanico") {
        if (telemetriaPotencia) telemetriaPotencia.textContent = "342.5 W";
      } else if (state.estadoMotor !== "parado") {
        const potMotor = (135.0 + (Math.random() * 2 - 1)).toFixed(1);
        if (telemetriaPotencia) telemetriaPotencia.textContent = potMotor + " W";
      } else {
        if (telemetriaPotencia) telemetriaPotencia.textContent = "0.8 W";
      }
    } else {
      if (breakerLever) {
      ...
      if (telemetriaVoltaje) telemetriaVoltaje.textContent = "0.0 V";
      if (telemetriaPotencia) telemetriaPotencia.textContent = "0.0 W";
    }

    // 2. Dispositivo Central
    ...
    if (svgDevLed) {
      ...
      } else if (state.averiaActiva === "bloqueo_mecanico" || state.averiaActiva === "cpulsar_bus_cortado") {
        svgDevLed.setAttribute("fill", "#ef4444");
      } else if (state.averiaActiva === "wifi_incompatible") {
    ...
    // 3. Relés K1 y K2
    if (state.alimentacion230v && state.releK1Cerrado) {
      if (svgContactK1) svgContactK1.classList.add("relay-closed-k1");
    ...
    if (state.alimentacion230v && state.releK2Cerrado) {
      if (svgContactK2) svgContactK2.classList.add("relay-closed-k2");
    ...
    if (wireOutN) {
      wireOutN.style.opacity = (state.alimentacion230v && state.estadoMotor !== "parado") ? "1" : "0";
    }

    // 4. Bornes Salida Subida / Bajada y Swap Giro
    ...
    if (state.alimentacion230v && state.estadoMotor !== "parado" && state.averiaActiva !== "bloqueo_mecanico") {
      const subiendoReal = state.sentidoInvertido
        ? (state.estadoMotor === "bajar")
        : (state.estadoMotor === "subiendo");
      ...
      if (svgMotorRotor) {
        ...
        svgMotorRotor.classList.add(subiendoReal ? "rotor-spin-ccw" : "rotor-spin-cw");
      }
      if (svgRotorTxt) {
        svgRotorTxt.textContent = subiendoReal ? "▲ SUBIENDO" : "▼ BAJANDO";
        svgRotorTxt.setAttribute("fill", subiendoReal ? "#5eead4" : "#38bdf8");
      }
      if (phoneMotorStateTxt) {
        phoneMotorStateTxt.textContent = subiendoReal ? "Subiendo..." : "Bajando...";
      }
    } else {
      if (svgMotorRotor) svgMotorRotor.classList.remove("rotor-spin-cw", "rotor-spin-ccw");
      if (svgRotorTxt) {
        svgRotorTxt.textContent = state.averiaActiva === "bloqueo_mecanico" ? "BLOQUEADO" : "PARADO";
        svgRotorTxt.setAttribute("fill", state.averiaActiva === "bloqueo_mecanico" ? "#ef4444" : "#64748b");
      }
      if (phoneMotorStateTxt) {
        phoneMotorStateTxt.textContent = state.averiaActiva === "bloqueo_mecanico" ? "¡Motor Bloqueado!" : "Parado";
      }
    }

    // 6. Router Wi-Fi & Nube
    const wifiOk = state.wifiActivo && state.bandaWifi === "2.4" && !state.cgnatActivo && state.averiaActiva !== "wifi_incompatible";
    if (svgWifiWaves) {
    ...
    if (svgRouterCgnatStatus) {
      svgRouterCgnatStatus.textContent = state.cgnatActivo || state.averiaActiva === "wifi_incompatible"
        ? "CG-NAT ACTIVO / 5 GHz (Error)"
        : "IP Pública Directa (Sin CG-NAT)";
      svgRouterCgnatStatus.setAttribute("fill", wifiOk ? "#38bdf8" : "#ef4444");
    }

    // 7. Smartphone Virtual
    const phoneOnline = state.alimentacion230v && wifiOk;
  ...
  // ---------- Actualización de Asesoría SAT en Vivo ----------
  function actualizarAsesoria(claveAveria) {
    ...
    if (asesoriaCaja) {
      if (diag.tipo === "peligro") {
        asesoriaCaja.className = "p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-xs text-rose-200 flex items-start gap-3 transition-all";
      } else if (diag.tipo === "alerta" || diag.tipo === "advertencia") {
        asesoriaCaja.className = "p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-xs text-amber-200 flex items-start gap-3 transition-all";
      } else {
        asesoriaCaja.className = "p-3.5 rounded-xl bg-cyan-500/10 border border-cyan-500/30 text-xs text-cyan-200 flex items-start gap-3 transition-all";
      }
    }
  }

  // ---------- Manejadores de Eventos del Dock y Cuadro ----------
  if (btnBreakerToggle) {
  ...
  btnsAverias.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (state.averiaActiva === averia) {
        ...
        if (averia === "fase_invertida") state.sentidoInvertido = false;
        if (averia === "corte_termico") state.alimentacion230v = true;
        if (averia === "wifi_incompatible") { state.bandaWifi = "2.4"; state.cgnatActivo = false; }
        if (averia === "bloqueo_mecanico") state.sobrecargaTermica = false;
      } else {
        state.averiaActiva = averia;
        if (averia === "fase_invertida") state.sentidoInvertido = true;
        if (averia === "corte_termico") { state.alimentacion230v = false; pararManiobra(); }
        if (averia === "wifi_incompatible") { state.bandaWifi = "5.0"; state.cgnatActivo = true; }
        if (averia === "cpulsar_bus_cortado") {
          state.dispositivo = "c-pulsar";
          if (selectDispositivo) selectDispositivo.value = "c-pulsar";
        }
        if (averia === "bloqueo_mecanico") { pararManiobra(); state.sobrecargaTermica = true; }
      }
      ...
      btnsAverias.forEach((b) => {
        ...
        if (b.dataset.averia === state.averiaActiva) {
          ...
          if (led) { led.className = "w-2.5 h-2.5 rounded-full bg-rose-500 shadow-sm averia-led animate-pulse"; }
        } else {
          b.classList.remove("border-rose-500/60", "bg-rose-950/20");
          if (led) { led.className = "w-2.5 h-2.5 rounded-full bg-slate-600 averia-led"; }
        }
      });
  ...
  if (btnPhoneSubir) {
    btnPhoneSubir.addEventListener("click", () => {
      const wifiOk = state.wifiActivo && state.bandaWifi === "2.4" && !state.cgnatActivo && state.averiaActiva !== "wifi_incompatible";
      if (!wifiOk || !state.alimentacion230v) return;
  ...
  // Pulsador Táctil de Marco C-Pulsar
  if (svgPulsadorMarco) {
  ...
  // Inicializar estado visual y arrancar bucle de animación
  actualizarVista();
  requestAnimationFrame(bucleAnimacion);
}
